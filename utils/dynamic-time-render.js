const dynamicRender = {
  ...
  computed: {
    blockWidth() {
      const { datas, cellWidth } = this;
      return datas.length * cellWidth;
    },
    //计算当前屏幕显示的第一行数据的index
    currentTopIndex() {
      const { scrollLeft, cellWidth, datas, widthOfBlocksWrapper } = this;
      const availableScrollLeft =
        datas.length * cellWidth - widthOfBlocksWrapper;
      if (cellWidth > 0 && availableScrollLeft < scrollLeft) {
        /*eslint no-console: "warn"*/
        console.log(
          `错误的scrollLeft值 ${scrollLeft},可用滚动高度为${availableScrollLeft}，代码自动矫正`
        );
        return Math.ceil(availableScrollLeft / cellWidth);
      }
      return Math.ceil(scrollLeft / cellWidth);
    },
    showDatas() {
     ...
     */
    topSpace() {
      const { oldTopIndex, cellWidth, preload } = this;
      const start = oldTopIndex - preload > 0 ? oldTopIndex - preload : 0;
      return start * cellWidth;
    }
  },
  ...
  methods: {
    ...
    sliceData() {
      ...
      //没有高度，不需要渲染元素
      if (widthOfBlocksWrapper === 0 || cellWidth === 0) {
        this.endRenderNum = 0;
      ...
      const endRenderNum =
        currentTopIndex +
        Math.ceil(widthOfBlocksWrapper / cellWidth) +
        preload;
      this.endRenderNum =
