    ...
    <ux-grid
      ...
      <ux-table-column
        ...
        <template v-slot="{row, rowIndex, $rowIndex, column, columnIndex, $columnIndex, _columnIndex}" class="progressCon">
          <div class="progressUpon" v-if="row.children[columnIndex]" :style="{'width': row.children[columnIndex].time/100 * 8 + 'px'}">{{row.children[columnIndex].time}}</div>
        </template>
      </ux-table-column>
    </ux-grid>
  </div>
  ...
  data() {
    return {
      ...
      mergeCells: [
        ...
        { row: 0, col: 1, rowspan: 1, colspan: 50 }
      ]
    }
  },
  mounted() {
